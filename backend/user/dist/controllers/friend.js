...
export const sendFriendRequest = TryCatch(async (req, res) => {
    ...
    const isAlreadyFriends = receiver.friends.includes(senderId);
    if (isAlreadyFriends) {
...
export const rejectFriendRequest = TryCatch(async (req, res) => {
    const userId = req.user?._id;
    const { requestId } = req.params;
    const request = await FriendRequest.findById(requestId);
    if (!request) {
...
export const getFriendRequests = TryCatch(async (req, res) => {
    const userId = req.user?._id;
